...
namespace DifferentialEquationCalculator.Model
{
    public class AdamsMethod
    {
        ...
        public Point<float>[] Solve(float left_border, float right_border, float y0, float step)
        {
            ...
            Point<float>[] F = _rungeKuttaMethod.Solve(left_border, left_border + 7 * step, y0, step);

            for (int i = 0; i < 4; i++)
            {
                points[i] = F[i];
                F[i] = new Point<float>(-1, _mathFunction.Calculate(F[i].X, F[i].Y));
            }
            ...
            {
                points[index + 1] = new Point<float>(left_border, points[index].Y + step / 24.0f 
                    * (55 * F[3].Y 
                    - 59 * F[2].Y 
                    + 37 * F[1].Y 
                    - 9 * F[0].Y));
                ...
                ShiftArray(F);
                F[3] = new Point<float>(-1, _mathFunction.Calculate(points[index + 1].X, points[index + 1].Y));
                left_border += step;
                index++;
            }
        ...
        private void ShiftArray(Point<float>[] points)
        {
